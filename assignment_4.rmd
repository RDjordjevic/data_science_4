...
author: |
  | Riccardo Djordjevic
  | r.djordjevicl@students.uu.nl
  | https://github.com/RDjordjevic
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
```{r}
library(modelsummary); library(tidyverse); library(plyr)


Growth_summary <- GrowthSW %>%
            mutate(
                   'treat' = ifelse(revolutions == 0, 0, 1)) 

datasummary(rgdp60 + growth ~ treat * (mean + sd + median + max + min), data = Growth_summary)

```
datasummary(growth + rgdp60 ~ treat * (growth_median + growth_sd + growth_min + growth_max + rgdp60_median + rgdp60_sd + rgdp60_min + rgdp60_max),
                    data = Growth_summary)
...
**Designated place** :Riccardo uses the function datasummary to fulfil the task above. He finds the mean, sd, median, max and min of the dataset Growth_summary. 
